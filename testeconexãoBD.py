...
# Função para adicionar uma fofoca
...
# Função para reportar uma fofoca
...
# Função para mostrar as fofocas
def mostrar_fofocas():
    conexao = conectar_banco()
    if conexao:
        cursor = conexao.cursor()
...
# Função para mostrar a tela de admin
...
# Função para salvar a fofoca
def salvar_fofoca(texto):
    adicionar_fofoca(texto, imagem_1, imagem_2, imagem_3)
    messagebox.showinfo("Sucesso", "Fofoca criada com sucesso!")

# Função para mostrar a tela de famoso
